...
// Generated from the reviewed final policy. ACTIONS_ATTRIBUTIONS permission lot only.
(function expose(root,factory){
  const api=factory();
  if(typeof module==="object"&&module.exports)module.exports=api;else root.PilotageActionAttributionPermissionLot=api;
})(typeof globalThis==="object"?globalThis:this,function factory(){
  const tableId="ACTIONS_ATTRIBUTIONS",expected=[
  {
    "key": "owner",
  ...
  {
    "key": "create-C",
    "aclFormula": "user.IsLoggedIn and user.Access == EDITOR and user.PilotageCompte.id > 0 and user.PilotageCompte.Actif and user.PilotageCompte.Interlocuteur > 0 and newRec.Attributaire == user.PilotageCompte.Interlocuteur and newRec.ACL_evenement_auteur == user.PilotageCompte.Interlocuteur and newRec.ACL_niveau_coherent and ((newRec.ACL_evenement_operation == 'create' and newRec.Niveau == 1) or (newRec.ACL_evenement_operation == 'assign' and newRec.Niveau == 2))",
    "permissionsText": "+C",
    "memo": "Opération métier vérifiée."
  },
  {
    "key": "assign-U",
    "aclFormula": "user.IsLoggedIn and user.Access == EDITOR and user.PilotageCompte.id > 0 and user.PilotageCompte.Actif and user.PilotageCompte.Interlocuteur > 0 and newRec.Attributaire == user.PilotageCompte.Interlocuteur and newRec.ACL_evenement_auteur == user.PilotageCompte.Interlocuteur and newRec.ACL_niveau_coherent and rec.Attributaire == user.PilotageCompte.Interlocuteur and rec.Niveau == 1 and rec.Revision_ecriture > 0 and newRec.Revision_ecriture > rec.Revision_ecriture and newRec.ACL_evenement_operation == 'assign' and (rec.Echeance == None or newRec.Echeance != None) and newRec.id == rec.id and newRec.Action == rec.Action and newRec.Niveau == rec.Niveau and newRec.Attributaire == rec.Attributaire and newRec.Destinataire == rec.Destinataire and newRec.Service_contexte == rec.Service_contexte and newRec.Date_attribution == rec.Date_attribution",
    "permissionsText": "+U",
    "memo": "Opération métier vérifiée."
  },
  {
