  ...
  function create({grist,mode,audit,lot,liveAudit=null}){
    ...
    return Object.freeze({
      ...
      async install({confirmed=false}={}){
        ...
        try{const current=await snapshot();if(JSON.stringify(current)!==preview){preview=null;throw Error("La structure ou les permissions ont changé : refaire la vérification.");}if(!audit.inspect(current).readyForPermissionReview)throw Error("Le contrôle préalable des permissions n’est plus conforme.");if(liveAudit){const before=await liveAudit.inspect();if(!before.readyForActionPolicy||JSON.stringify(before.counts)!==livePreview)throw Error("Les compteurs historiques ont changé : refaire les contrôles.");}const result=lot.inspect(current);if(!result.readyToInstall||!result.actions.length)throw Error(`Lot ${lot.tableId} bloqué.`);await guard();preview=null;outcomeUncertain=true;await grist.docApi.applyUserActions(result.actions);const after=await snapshot(),verified=lot.inspect(after);if(!verified.alreadyInstalled||outside(current)!==outside(after))throw Error("La relecture ne confirme pas une modification isolée de la table attendue.");if(liveAudit){const afterLive=await liveAudit.inspect();if(!afterLive.readyForActionPolicy||JSON.stringify(afterLive.counts)!==livePreview)throw Error("La relecture des actions historiques n’est pas conforme.");}outcomeUncertain=false;return report(verified);}catch(error){if(outcomeUncertain)throw Error(`Ne pas relancer automatiquement. ${error.message}`);throw error;}finally{busy=false;}
      }
    });
  }
