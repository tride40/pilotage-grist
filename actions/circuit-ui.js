...
(function(root){
  ...
  function mount({element,service,catalog={projects:[],people:[],services:[],poles:[]},canWrite=false,allowCreate=canWrite,allowAssignment=canWrite,allowLifecycle=canWrite,confirmWrites=false,banner="Écritures désactivées : activation du circuit en attente.",title="Circuit des actions",initialFilter="open",showCreate=true,showNotifications=false}){
    ...
    function isOverdue(row){if(!row.deadline||["performed","closed","cancelled"].includes(row.state))return false;const end=new Date(`${row.deadline}T23:59:59`);return !Number.isNaN(end.getTime())&&end.getTime()<Date.now();}
    ...
    async function openDetail(row){
      ...
      try{const fresh=await service.inspect(row.id);selected=fresh.row;opener=doc.activeElement;form.reset();fields.replaceChildren();error.textContent="";formTitle.textContent="Détail de l’action";currentCapability=false;currentOperation="view";showSubmit(false);back.textContent="Retour à la liste";
        const summary=group(selected.title);detailLine(summary,"Projet",selected.projectTitle);detailLine(summary,"État",labels[selected.state]||selected.state);detailLine(summary,"Échéance effective",formatDate(selected.deadline));if(isOverdue(selected))detailLine(summary,"Situation","En retard");detailLine(summary,"Dernière mise à jour",formatDate(selected.updatedAt,true));
        ...
        if(selected.deadlineChain?.some(item=>item.date)){const calendar=group("Parcours des échéances");selected.deadlineChain.forEach((item,index)=>{if(item.date)detailLine(calendar,`Niveau ${index+1} · ${personName(item.personId)}`,formatDate(item.date));});}
        const outcomeValues=[["Bilan",selected.result],["Motif du complément",selected.additionalWorkReason],["Motif de l’annulation",selected.cancellationReason],["Déclarée réalisée le",formatDate(selected.performedAt,true)],["Déclarée réalisée par",selected.performedBy?personName(selected.performedBy):null],["Clôturée le",formatDate(selected.closedAt,true)],["Annulée le",formatDate(selected.cancelledAt,true)]];
