...
// Recette only. Grist ACLs remain the authority; client checks also protect
...
(function expose(root, factory) {
  ...
  function create({ grist, mode, identify }) {
    ...
    async function context() {
      guard();
      if (await grist.docApi.getDocName() !== RECIPE) throw Error("Cette page est réservée au document de recette autorisé.");
      let accounts;
      try { accounts = rows(await grist.docApi.fetchTable("PILOTAGE_COMPTES")); }
      catch { throw Error("Identification indisponible : la lecture de votre propre compte doit être autorisée dans la recette."); }
  ...
  return Object.freeze({ create, recipeDocumentId: RECIPE });
});
