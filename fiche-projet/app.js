...
async function fetchMetadata() {
  try {
    ...
    const details = Object.fromEntries(["PROJETS", "AVANCEMENTS", "ARBITRAGES_DECISIONS","JALONS","BLOCAGES","VIGILANCES"].map((table) => [table, new Map(byTable(table).map((column) => [column.colId, column]))]));
...
function renderProgress(project) {
  ...
  const alerts=[...blockages.slice(0,3).map(row=>["BLOCAGES",row,"Blocage",row.Blocage,"danger"]),...vigilances.slice(0,3).map(row=>["VIGILANCES",row,"Vigilance",row.Vigilance,"warning"])];if(alerts.length||milestones.length){const list=element("div","situation-list");alerts.forEach(([table,row,label,value,tone])=>{const item=element("div",`alert alert--${tone}`);item.append(textElement("strong",label),textElement("span",value));const button=textElement("button",table==="BLOCAGES"?"Marquer comme levé":"Lever la vigilance","button button--secondary");button.type="button";button.addEventListener("click",()=>resolvePilotageObject(table,row));item.append(button);list.append(item)});milestones.slice(0,3).forEach(row=>{const item=element("div","alert alert--info");item.append(textElement("strong","Jalon"),textElement("span",row.Jalon));if(hasValue(row.Date_prevue))item.append(textElement("small",formatDate(row.Date_prevue)));const button=textElement("button","Marquer comme franchi","button button--secondary");button.type="button";button.addEventListener("click",()=>resolvePilotageObject("JALONS",row));item.append(button);list.append(item)});ui.progress.append(list)}
