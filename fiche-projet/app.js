...
const appState = { selectedProject: null, tables: null, tableNames: [], metadata: null, currentUser: null, demo: false, busy: false, editingJournalId: null, editingDecisionId: null, editingMilestoneId: null, journalActionSource: null, deletingJournalId: null, journalSearch: "", journalTypeFilter: "", journalStateFilter: "", showAllJournal: false };
const ui = {
  ...
  projectContactDialog: document.querySelector("#project-contact-dialog"), projectContactForm: document.querySelector("#project-contact-form"),
...
const activeProjectPickers = new Map();
...
async function fetchMetadata() {
  try {
    ...
    const details = Object.fromEntries(["PROJETS", "INTERLOCUTEURS", "AVANCEMENTS", "ARBITRAGES_DECISIONS","JALONS","BLOCAGES","VIGILANCES","ATTENTES_EXTERNES","RELANCES_ATTENTES"].map((table) => [table, new Map(byTable(table).map((column) => [column.colId, column]))]));
    ...
    return { details: {}, writable: { PROJETS: new Set(Object.keys(appState.tables.PROJETS?.[0] || {})), INTERLOCUTEURS: new Set(Object.keys(appState.tables.INTERLOCUTEURS?.[0] || {})), AVANCEMENTS: new Set(Object.keys(appState.tables.AVANCEMENTS?.[0] || {})), ARBITRAGES_DECISIONS: new Set(Object.keys(appState.tables.ARBITRAGES_DECISIONS?.[0] || {})) }, choices: Object.fromEntries(PROJECT_CHOICE_FIELDS.map((field) => [field, []])), warning: exactError(error) };
  }
}
...
function renderHeroPeople(project) {
  ...
  appendDefinition(ui.people,"Agents associés",personValue(project.Agents_associes));
  appendDefinition(ui.people,"Élus associés",personValue(project.Elus_associes));
  appendDefinition(ui.people,"Interlocuteurs externes",personValue(project.Interlocuteurs_externes));
  appendDefinition(ui.people,"Services concernés",serviceValue(project.Services_concernes));
...
const PROJECT_FIELDS = [
  ...
  ["Agents_associes","Agents associés","agents"],["Elus_associes","Élus associés","elus"],["Interlocuteurs_externes","Interlocuteurs externes","externes"],["Services_concernes","Services concernés","services"],
...
function bindEditing() {
  ...
  ui.projectContactForm.addEventListener("submit", saveProjectContact);
  ui.projectContactForm.elements.Interne_Mairie.addEventListener("change", refreshProjectContactProfile);
...
function projectField(name){
  ...
  if(name==="Mois_lancement")return projectSelectField(name,label,["Janvier","Février","Mars","Avril","Mai","Juin","Juillet","Août","Septembre","Octobre","Novembre","Décembre"],appState.selectedProject[name]);
  if(name==="Trimestre_objectif")return projectSelectField(name,label,["T1","T2","T3","T4"],appState.selectedProject[name]);
  const field=formField(name,label,type,appState.selectedProject[name]);if(name==="Statut")field.classList.add("form-field--compact");return field;
}
function projectSelectField(name,label,values,value){const wrapper=element("label","form-field"),select=document.createElement("select");wrapper.append(textElement("span",label,"form-field__label"));select.className="form-field__control";select.name=name;select.append(option("","Non renseigné"),...values.map(item=>option(item,item)));select.value=displayValue(value);wrapper.append(select);return wrapper;}
...
function choicePanel(name,label,values,current,help){const fieldset=element("fieldset","form-field form-field--wide choice-panel"),legend=textElement("legend",label,"form-field__label"),grid=element("div","choice-grid"),selected=new Set(referenceIds(current));for(const entry of values){const value=typeof entry==="object"?entry.value:entry,labelText=typeof entry==="object"?entry.label:entry,item=element("label","choice-option"),input=document.createElement("input");input.type="checkbox";input.name=name;input.value=value;input.checked=selected.has(String(value));item.append(input,textElement("span",labelText));grid.append(item)}fieldset.append(legend,grid,textElement("small",help));return fieldset;}
function projectPeople(kind){return(appState.tables.INTERLOCUTEURS||[]).filter(person=>!Object.hasOwn(person,"Actif")||isTrue(person.Actif)).filter(person=>{const internal=isTrue(person.Interne_Mairie??person.Interne_Sanguinet),role=normalizeText(person.Role_interne),agent=role==="agent"||isTrue(person.Est_agent_Sanguinet),elected=role==="elu"||isTrue(person.Est_elu_Sanguinet);if(kind==="agents")return agent;if(kind==="elus")return elected;return !internal&&!agent&&!elected})}
function projectPersonLabel(person){const full=[person.Prenom,person.Nom].filter(hasValue).join(" ");return hasValue(person.Nom_complet)?displayValue(person.Nom_complet):full||`Interlocuteur ${person.id}`}
function projectPersonPicker(name,label,kind,current){
  const fieldset=element("fieldset","form-field form-field--wide people-picker"),legend=textElement("legend",label,"form-field__label"),selectedBox=element("div","people-picker__selected"),search=document.createElement("input"),results=element("div","people-picker__results");
  const selected=new Set(referenceIds(current));search.type="search";search.className="form-field__control people-picker__search";search.placeholder=`Rechercher ${kind==="elus"?"un élu":kind==="agents"?"un agent":"un interlocuteur externe"}…`;search.autocomplete="off";results.hidden=true;results.setAttribute("role","listbox");
  const renderSelected=()=>{selectedBox.replaceChildren();if(!selected.size)selectedBox.append(textElement("p","Aucune personne sélectionnée.","people-picker__empty"));for(const id of selected){const person=(appState.tables.INTERLOCUTEURS||[]).find(row=>String(row.id)===String(id)),chip=element("span","person-chip"),hidden=document.createElement("input"),remove=document.createElement("button");hidden.type="hidden";hidden.name=name;hidden.value=id;remove.type="button";remove.className="person-chip__remove";remove.setAttribute("aria-label",`Retirer ${person?projectPersonLabel(person):id}`);remove.textContent="×";remove.addEventListener("click",()=>{selected.delete(String(id));renderSelected();renderResults(search.value)});chip.append(textElement("span",person?projectPersonLabel(person):`Interlocuteur ${id}`),remove,hidden);selectedBox.append(chip)}};
  const renderResults=(query,force=false)=>{const needle=normalizeText(query);results.replaceChildren();if(!force&&!needle){results.hidden=true;return}const matches=projectPeople(kind).filter(person=>!selected.has(String(person.id))).filter(person=>!needle||normalizeText([projectPersonLabel(person),person.Fonction,person.Organisme].filter(hasValue).join(" ")).includes(needle)).slice(0,10);if(!matches.length)results.append(textElement("p","Aucun résultat.","people-picker__empty"));for(const person of matches){const button=document.createElement("button");button.type="button";button.className="people-picker__result";button.setAttribute("role","option");button.append(textElement("strong",projectPersonLabel(person)),textElement("small",[person.Fonction,person.Organisme].filter(hasValue).join(" · ")||"Interlocuteur"));button.addEventListener("click",()=>{selected.add(String(person.id));search.value="";results.hidden=true;renderSelected();search.focus()});results.append(button)}results.hidden=false};
  search.addEventListener("input",()=>renderResults(search.value));search.addEventListener("focus",()=>{if(search.value)renderResults(search.value)});renderSelected();fieldset.append(legend,selectedBox,search,results,textElement("small","Recherchez une personne, puis cliquez sur son nom pour l’ajouter."));activeProjectPickers.set(name,{add(id){selected.add(String(id));search.value="";results.hidden=true;renderSelected()},refresh(){renderSelected();renderResults(search.value)}});return fieldset;
}
function openProjectContactForm(){ui.projectContactForm.reset();ui.projectContactDialog.querySelector(".form-message").textContent="";refreshProjectContactProfile();ui.projectContactDialog.showModal();ui.projectContactForm.elements.Prenom.focus()}
function refreshProjectContactProfile(){const internal=ui.projectContactForm.elements.Interne_Mairie.checked,role=ui.projectContactForm.elements.Role_interne,organisation=ui.projectContactForm.elements.Organisme;document.querySelector("#project-contact-role").hidden=!internal;role.required=internal;organisation.required=!internal;organisation.closest("label").querySelector(".form-field__label").textContent=internal?"Organisme":"Organisme *";if(!internal)role.value=""}
function openProjectForm() {
  activeProjectPickers.clear();
  ...
  ui.editFields.replaceChildren(
    ...
    formSection(2,"Classification","Statut et domaines concernés",fieldGrid([projectField("Statut"),projectHasColumn("Thematiques")?choicePanel("Thematiques","Thématiques",projectChoiceValues("Thematiques"),appState.selectedProject.Thematiques,"Sélectionnez une ou plusieurs thématiques."):null,projectField("Motif_abandon")])),
    formSection(3,"Pilotage","Responsables administratif et politique",fieldGrid([projectField("Agent_pilote"),projectField("Responsable"),projectField("Elu_pilote")])),
    formSection(4,"Équipe associée","Personnes et services qui contribuent au projet",fieldGrid([projectHasColumn("Agents_associes")?projectPersonPicker("Agents_associes","Agents associés","agents",appState.selectedProject.Agents_associes):null,projectHasColumn("Elus_associes")?projectPersonPicker("Elus_associes","Élus associés","elus",appState.selectedProject.Elus_associes):null,projectHasColumn("Interlocuteurs_externes")?projectPersonPicker("Interlocuteurs_externes","Interlocuteurs externes","externes",appState.selectedProject.Interlocuteurs_externes):null,projectHasColumn("Services_concernes")?choicePanel("Services_concernes","Services concernés",(appState.tables.SERVICES||[]).filter(row=>!Object.hasOwn(row,"Actif")||isTrue(row.Actif)).map(row=>({value:String(row.id),label:textOr(row.Nom_service||row.Nom,`Service ${row.id}`)})),appState.selectedProject.Services_concernes,"Sélectionnez les services concernés."):null,(()=>{const action=element("div","form-field--wide form-inline-action"),button=document.createElement("button");button.type="button";button.className="button button--secondary";button.textContent="+ Créer un interlocuteur";button.addEventListener("click",openProjectContactForm);action.append(button,textElement("small","La nouvelle personne sera créée sans fermer cette fiche, puis ajoutée automatiquement au bon groupe."));return action})()])),
    formSection(5,"Calendrier","Lancement et objectif de réalisation",calendar)
  );
...
function formField(name,label,type,value){const multiple=["choicelist","agents","elus","externes","services"].includes(type),wrapper=element("label",`form-field${type==="textarea"||multiple?" form-field--wide":""}`);wrapper.append(textElement("span",label,"form-field__label"));let control;if(type==="textarea"){control=document.createElement("textarea");control.rows=3;}else if(type==="choice"||type==="choicelist"){control=document.createElement("select");const values=projectChoiceValues(name);if(type==="choice")control.append(option("","Non renseigné"));control.append(...values.map((item)=>option(item,item)));}else if(["agent","elu","agents","elus","externes","services"].includes(type)){control=document.createElement("select");if(type==="services")control.append(...(appState.tables.SERVICES||[]).filter(row=>!Object.hasOwn(row,"Actif")||isTrue(row.Actif)).map(row=>option(row.id,textOr(row.Nom_service||row.Nom,`Service ${row.id}`))));else{const role=type.startsWith("agent")?"agent":type.startsWith("elu")?"elu":"",people=(appState.tables.INTERLOCUTEURS||[]).filter(person=>!Object.hasOwn(person,"Actif")||isTrue(person.Actif)).filter(person=>role?normalizeText(person.Role_interne)===role||isTrue(person[role==="agent"?"Est_agent_Sanguinet":"Est_elu_Sanguinet"]):!isTrue(person.Interne_Mairie??person.Interne_Sanguinet));control.append(...(!multiple?[option("","Non renseigné")]:[]),...people.map(p=>option(p.id,textOr(p.Nom_complet,`Interlocuteur ${p.id}`))))}}else{control=document.createElement("input");control.type=type;if(type==="number"){control.min=2000;control.max=2200;}}control.className="form-field__control";control.name=name;if(multiple){control.multiple=true;control.size=Math.min(6,Math.max(2,control.options.length));const selected=new Set(referenceIds(value));[...control.options].forEach(item=>{item.selected=selected.has(item.value)})}else control.value=type==="date"?dateInputValue(value):displayValue(value);wrapper.append(control);return wrapper;}
...
async function saveProjectContact(event){
  event.preventDefault();if(appState.busy)return;const message=ui.projectContactDialog.querySelector(".form-message"),data=new FormData(ui.projectContactForm),internal=data.has("Interne_Mairie"),role=String(data.get("Role_interne")||"").trim(),name=String(data.get("Nom")||"").trim();message.textContent="";
  try{
    if(!name)throw new Error("Renseignez le nom de l’interlocuteur.");if(internal&&!role)throw new Error("Choisissez si la personne est un agent ou un élu.");
    const contactWritable=field=>appState.demo||tableWritable("INTERLOCUTEURS",field),values={};for(const field of ["Prenom","Nom","Organisme","Fonction","Email","Telephone","Notes"])if(contactWritable(field))values[field]=String(data.get(field)||"").trim();
    const internalField=contactWritable("Interne_Mairie")?"Interne_Mairie":contactWritable("Interne_Sanguinet")?"Interne_Sanguinet":"";if(!internalField)throw new Error("La colonne de profil interne de la table INTERLOCUTEURS n’est pas modifiable.");values[internalField]=internal;
    if(contactWritable("Role_interne"))values.Role_interne=internal?role:"";if(contactWritable("Actif"))values.Actif=true;if(appState.demo){values.Nom_complet=[values.Prenom,values.Nom].filter(hasValue).join(" ");values.Est_agent_Sanguinet=internal&&normalizeText(role)==="agent";values.Est_elu_Sanguinet=internal&&normalizeText(role)==="elu"}
    const errors=window.PilotageV3Rules?.personErrors?.(values)||[];if(errors.length)throw new Error(errors[0]);
    const before=new Set((appState.tables.INTERLOCUTEURS||[]).map(person=>String(person.id)));appState.busy=true;ui.projectContactForm.querySelectorAll("input,textarea,select,button").forEach(control=>control.disabled=true);
    if(appState.demo)applyDemoActions([["AddRecord","INTERLOCUTEURS",null,values]]);else{await window.PilotageGristWrite.applyUserActions([["AddRecord","INTERLOCUTEURS",null,values]]);appState.tables=await fetchRelatedTables()}
    const created=(appState.tables.INTERLOCUTEURS||[]).find(person=>!before.has(String(person.id)));if(!created)throw new Error("L’interlocuteur a été créé mais n’a pas pu être identifié automatiquement.");
    const pickerName=internal?(normalizeText(role)==="elu"?"Elus_associes":"Agents_associes"):"Interlocuteurs_externes";for(const picker of activeProjectPickers.values())picker.refresh();activeProjectPickers.get(pickerName)?.add(created.id);
    ui.projectContactDialog.close();ui.editDialog.querySelector(".form-message").textContent=`${projectPersonLabel(created)} a été créé et ajouté au projet.`;
  }catch(error){message.textContent=`Création impossible — ${exactError(error)}`}finally{appState.busy=false;ui.projectContactForm.querySelectorAll("input,textarea,select,button").forEach(control=>control.disabled=false)}
}
...
function openMilestoneForm(row=null){appState.editingMilestoneId=row?.id??null;ui.milestoneForm.reset();ui.milestoneFields.replaceChildren();ui.milestoneDialog.querySelector("#milestone-title").textContent=row?"Modifier le jalon":"Nouveau jalon";ui.milestoneDialog.querySelector("[type=submit]").textContent=row?"Enregistrer":"Créer le jalon";const fields=[];if(tableWritable("JALONS","Jalon")){const title=formField("Jalon","Jalon *","textarea",row?.Jalon);title.querySelector("textarea").required=true;fields.push(title)}if(tableWritable("JALONS","Date_prevue"))fields.push(formField("Date_prevue","Date prévue","date",row?.Date_prevue));if(tableWritable("JALONS","A_retenir"))fields.push(formField("A_retenir","À retenir","textarea",row?.A_retenir));ui.milestoneFields.append(...fields);ui.milestoneDialog.querySelector(".form-message").textContent="";ui.milestoneDialog.showModal();ui.milestoneForm.elements.Jalon?.focus()}
async function saveMilestone(event){event.preventDefault();const message=ui.milestoneDialog.querySelector(".form-message");try{for(const column of ["Projet","Jalon"])if(!tableWritable("JALONS",column))throw new Error(`JALONS.${column} doit être une colonne modifiable.`);const data=new FormData(ui.milestoneForm),values={Projet:appState.selectedProject.id,Jalon:String(data.get("Jalon")||"").trim()};if(!values.Jalon)throw new Error("Renseignez le jalon.");if(tableWritable("JALONS","Date_prevue"))values.Date_prevue=hasValue(data.get("Date_prevue"))?new Date(`${data.get("Date_prevue")}T00:00:00`).getTime()/1000:null;if(tableWritable("JALONS","A_retenir"))values.A_retenir=String(data.get("A_retenir")||"").trim();if(!appState.editingMilestoneId&&tableWritable("JALONS","Franchi"))values.Franchi=false;const action=appState.editingMilestoneId?["UpdateRecord","JALONS",appState.editingMilestoneId,values]:["AddRecord","JALONS",null,values];await writeChanges(ui.milestoneDialog,[action],appState.editingMilestoneId?"Jalon mis à jour.":"Jalon créé et lié au projet.");appState.editingMilestoneId=null}catch(error){message.textContent=`Enregistrement impossible — ${exactError(error)}`}}
...
function serviceValue(value){if(!hasValue(value))return"";const services=appState.tables?.SERVICES||[];return referenceIds(value).map(identifier=>{const service=services.find(candidate=>String(candidate.id)===String(identifier));return textOr(service?.Nom_service||service?.Nom,identifier)}).join(", ")}
